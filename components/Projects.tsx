...
const ProjectCard: React.FC<{ project: Project; index: number; language: Language }> = ({ project, index, language }) => {
  ...
  return (
    <div
      ref={cardRef}
      className={`group bg-white dark:bg-slate-800/30 border border-slate-200 dark:border-slate-700/50 rounded-2xl overflow-hidden hover:shadow-2xl hover:shadow-primary/10 transition-all duration-300 flex flex-col ${isVisible ? 'animate-fade-in-up opacity-100' : 'opacity-0 translate-y-10'
        }`}
          ...
          <div className="flex gap-3">
            {project.demoUrl && (
              <a
                ...
                className="p-2 bg-primary rounded-full hover:bg-primary/80 transition-colors text-white"
                title="Live Demo"
              >
            ...
            {project.repoUrl && (
              <a href={project.repoUrl} className="p-2 bg-slate-700 rounded-full hover:bg-slate-600 transition-colors text-white" title="View Code">
                <Github size={18} />
              </a>
            )}
          </div>
      ...
      <div className="p-6 flex-1 flex flex-col">
        ...
        <div className="flex flex-wrap gap-2 mt-auto">
          {project.tags.map((tag) => (
            <span key={tag} className="px-2 py-1 text-xs font-medium text-slate-600 dark:text-slate-300 bg-slate-100 dark:bg-slate-700/50 rounded border border-slate-200 dark:border-slate-600">
              {tag}
            </span>
          ))}
        </div>
  ...
  return (
    <section id="projects" className="py-20 relative transition-colors duration-300">
      <div className="container mx-auto px-6">
        ...
        <div className="flex flex-wrap justify-center gap-2 mb-12 animate-fade-in-up">
          <button
            ...
            className={`px-4 py-2 rounded-full text-sm font-medium transition-all duration-300 ${activeFilter === 'All'
              ...
              : 'bg-white dark:bg-slate-800 text-slate-600 dark:text-slate-400 hover:bg-slate-50 dark:hover:bg-slate-700 border border-slate-200 dark:border-slate-700 hover:border-primary/50'
              }`}
          >
            {t.filterAll}
          </button>
        ...
        <div className="bg-white/80 dark:bg-slate-800/30 border border-slate-200 dark:border-slate-700/50 rounded-2xl p-8 lg:p-10 shadow-sm dark:shadow-none">
          <div className="flex flex-col md:flex-row items-start md:items-center justify-between mb-8 gap-4">
            ...
            <button className="text-primary hover:text-slate-900 dark:hover:text-white text-sm font-medium transition-colors">{t.viewAll} &rarr;</button>
          </div>
              ...
              const Content = (
                <>
                  ...
                  <div className="flex-1">
                    <h4 className="text-slate-900 dark:text-white font-semibold text-sm line-clamp-1 group-hover:text-primary transition-colors">{cert.title}</h4>
